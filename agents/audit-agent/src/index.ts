...
const logger = new Logger('AuditAgentMain');

/**
 * Simple EventBus implementation for the Audit Agent
 */
...
async function main() {
  try {
    ...
    const agent = new AuditAgent({
      eventBus,
      mongodbUrl: process.env.MONGO_URL || 'mongodb://localhost:27017',
      rabbitmqUrl: process.env.RABBITMQ_URL || 'amqp://localhost',
    });

    await agent.initialize();
    logger.info('✓ Audit Agent started successfully');
    ...
    logger.error('Failed to start Audit Agent', error);
    process.exit(1);
  }
}
