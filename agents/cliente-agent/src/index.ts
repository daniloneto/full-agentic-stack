...
import { ClienteAgent } from './cliente-agent.js';
import amqp from 'amqplib';

const logger = new Logger('ClienteAgentMain');

/**
 * Simple EventBus implementation for the Cliente Agent
 */
...
async function main() {
  try {
    ...
    logger.info('✓ Cliente Agent started successfully');
    ...
    logger.error('Failed to start Cliente Agent', error);
    process.exit(1);
  }
}
